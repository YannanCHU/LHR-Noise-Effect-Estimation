...
filename = "..\OS_Maps\Heathrow Airport\East Part1\map_5_16.png";
...
distMat = sqrt((x-cent(2)).^2 + (y-cent(1)).^2);
indexOfFreWithinRange = (distMat>lowFreThreshold) & (distMat<highFreThreshold);
...
%     figure(); imshow(region_candidate_BW); title("After morphological operation"); daspect([1,1,1]);   impixelinfo;
    ...
    info = regionprops(region_candidate_lb, 'Area', 'BoundingBox', 'Orientation');
    ...
    [region_candidate_lb_new, invalidFlag] = validityCheck(info, mapBinary, region_candidate_lb, spatialPeriod_1st, orientation_1st);
...
if numOfInvalidPeaks == ceil(length(XMAX)/2)
    ...
    figure();    imshow(map_masked2);     title("All regions of interest are superimposed");
end



% functions
function [region_candidate_lb, invalidFlag] = validityCheck(info, mapBinary, region_candidate_lb, spatialPeriod_1st, orientation_1st)
...
% for peakIndex = 3
    ...
    regionDetectedFFT = fftshift(fft2(regionDetected));
    regionDetectedMag = abs(regionDetectedFFT);
    regionDetectedPhase = angle(regionDetectedFFT);

    [~, y_index2, x_index2] = maxkVals(regionDetectedMag, 3);
    cent2 = size(regionDetectedMag)/2+1;
    spatialFrequency_2rd = sqrt(sum(((y_index2(2) - cent2(1)) / numel(yrange)).^2 + ((x_index2(2) - cent2(2)) / numel(xrange)).^2));
    ...
    orientation_diff = abs(orientation_2rd - orientation_1st);
...
%     fprintf("The first measured orientation is %2.3f, the second measured orientation is %2.3f.\n", orientation_1st, orientation_2rd);
    if spatialPeriod_diff >= 0.05 || orientation_diff > 10
